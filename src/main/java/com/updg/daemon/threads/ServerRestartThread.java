 ...
 * Date: 09.11.13  18:52
 */
public class ServerRestartThread extends Thread implements Runnable {
    private ServerConfig config;

    public ServerRestartThread(ServerConfig config) {
        this.config = config;
    }

    public void run() {
        Daemon.servers.get(config.getId()).setStoping(true);
        Daemon.servers.get(config.getId()).exec("say СЕРВЕР ПЕРЕЗАГРУЗИТСЯ ЧЕРЕЗ 15 СЕКУНД!");
        int i = 15;
        while (i > 0) {
            try {
                Thread.sleep(1000);
            ...
            i--;
            if (i == 10)
                Daemon.servers.get(config.getId()).exec("say СЕРВЕР ПЕРЕЗАГРУЗИТСЯ ЧЕРЕЗ 10 СЕКУНД!");
            if (i == 5)
                Daemon.servers.get(config.getId()).exec("say СЕРВЕР ПЕРЕЗАГРУЗИТСЯ ЧЕРЕЗ 5 СЕКУНД!");
        }
